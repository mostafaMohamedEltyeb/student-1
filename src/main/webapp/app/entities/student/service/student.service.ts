...
export class StudentService {
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    lated(req?: any): Observable<EntityArrayResponseType> {
      const options = createRequestOption(req);
      return this.http
        .get<IStudent[]>(this.resourceUrl + '/lated', { params: options, observe: 'response' })
        .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
    }
  ...
  protected convertDateFromClient(student: IStudent): IStudent {
    return Object.assign({}, student, {
      joiningDate: student.joiningDate?.isValid() ? student.joiningDate.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((student: IStudent) => {
        student.joiningDate = student.joiningDate ? dayjs(student.joiningDate) : undefined;
      });
    }
    return res;
  }
